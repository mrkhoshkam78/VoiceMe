/**
 * Audio Editor V2.5.0 – Application (UI layer only) — effect toggle FIXED
...
const ICONS = {
  ...
  vocal: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M12 2a3 3 0 0 1 3 3v6a3 3 0 0 1-6 0V5a3 3 0 0 1 3-3z"/><path d="M19 11a7 7 0 0 1-14 0M12 18v4M8 22h8"/><path d="M4 4l16 16"/></svg>'
};

const CATEGORIES = { voice: 'صدا', environment: 'محیط', enhancement: 'بهبود', studio: 'استودیو' };
...
class App {
  ...
  _renderControls(id) {
    ...
    if (id === 'femaleVoice') {
      ...
      body += this._chips(id, 'quality', 'کیفیت پردازش', [
        { v: 'fast', l: 'سریع' }, { v: 'high', l: 'کیفیت بالا' }
      ], p.quality || 'high');
      ...
      body += `<div class="control-row" style="margin-top:0.75rem">
        ...
        <p style="font-size:0.75rem;color:var(--text-muted);margin-top:0.5rem;line-height:1.4">
          پردازش Offline است و ممکن است چند ثانیه تا چند دقیقه طول بکشد.
          پس از اتمام می‌توانید Original و Instrumental را مقایسه کنید.
        </p>
      </div>`;
  ...
  async _runVocalRemoval() {
    ...
    try {
      const result = await this.engine.processVocalRemoval(params, (p, label) => {
        ...
        this.$.overlayText.textContent = label || ('جداسازی... ' + Math.round(p * 100) + '٪');
      });
      this._hideOverlay();
      this._syncEffects();
      this._updateChain();
      ...
      if (conf < 30) {
        this._toast('warning', 'جداسازی ضعیف',
          'اطمینان پایین (' + conf + '٪). روی این فایل ممکن است Vocal باقی بماند.');
      } else {
        this._toast('success', 'Instrumental آماده است',
          'اطمینان تقریبی ' + conf + '٪ — با A/B مقایسه کنید');
      }
  ...
  _ensureVocalAB() {
    ...
    this.$.modeProcessed.classList.remove('active');
    ...
    btnV.addEventListener('click', () => {
      if (!this.engine.setPlaybackSource('vocals')) {
        this._toast('warning', 'موجود نیست', 'Stem وکال برای این فایل ساخته نشد');
        return;
      }
      switchEl.querySelectorAll('button').forEach(b => b.classList.remove('active'));
      btnV.classList.add('active');
    });
    this.$.modeOriginal.addEventListener('click', () => {
      this.engine.setPlaybackSource('original');
      switchEl.querySelectorAll('button').forEach(b => b.classList.remove('active'));
      this.$.modeOriginal.classList.add('active');
    });
  }
